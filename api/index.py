...
# Adjust sys.path to allow importing from parent directory on Vercel
...
def calculate_cluster_metrics(df, features, assignments, k):
    try:
        ...
        dbi = float(davies_bouldin_score(X, assignments)) if len(np.unique(assignments)) > 1 else 0.0
        sil = float(silhouette_score(X, assignments)) if len(np.unique(assignments)) > 1 else 0.0
        dist = {str(i): {"count": int(np.sum(assignments == i)), "percentage": float(np.sum(assignments == i) / len(df) * 100)} for i in range(k)}
        profiles = {str(i): df[assignments == i][features].mean().to_dict() for i in range(k)}
...
@app.get("/api/health")
async def health():
    return {"status": "UP", "engine": "Vercel Serverless"}
...
async def run_kmeans_step(x_session_id: Optional[str] = Header(None), params: Dict[str, Any] = Body({"k": 3})):
    ...
    df['cluster'] = clusters
    for i in range(k): df[f"dist_c{i}"] = np.linalg.norm(X.values - model.cluster_centers_[i], axis=1)
